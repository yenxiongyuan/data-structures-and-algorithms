...
1. Write a function named transformToLis that, given an object,
2. returns an array of the key value pairs as html list items.
...
function transformToLis(obj){
  ...
  return Object.entries(obj).map(
    (property) => `<li>${property[0]}: ${property[1]}</li>`
  );
}
...
1. Write a function named count that, given an integer and an array of arrays,
2. uses either filter, map, or reduce to count the amount of times the integer is present in the array of arrays.
...
1. Write a function that, given an array of integer arrays as input,
2. calculates the total sum of all the elements in the array.
...
1. Write a function named divisibleByFiveTwoToThePower that accepts an array of arrays as input.

2. This function should first remove any elements that are not numbers or are not divisible by five.

3. This function should then raise 2 to the power of the resulting numbers, returning an array of arrays.
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(obj => obj.gender.includes('male'))
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortestSoFar, nextPerson) =>
    +shortestSoFar.height < +nextPerson.height ? shortestSoFar : nextPerson
  ).name;
};
